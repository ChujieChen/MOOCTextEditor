...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		boolean foundWord2 = word1.equals(word2);
		...
		while(!queue.isEmpty() && !foundWord2) {
			...
			for(String neighbor : neighbors) {
				if(!visited.contains(neighbor)) {
					WPTreeNode next = curr.addChild(neighbor);
					queue.add(next);
...
//						System.out.println(printQueue(queue));
	...
	public static void main(String[] args) {
		WPTree wTree = new WPTree();
		List<String> path = wTree.findPath("smart", "small");
		System.out.println(path.toString());
		
//		path = wTree.findPath("the", "kangaroo");
//		System.out.println(path.toString());
	}
